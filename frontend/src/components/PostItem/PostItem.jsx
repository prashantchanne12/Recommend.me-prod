...
import {
  ...
  removeUpvoteRecommendation,
  shareAction,
  addRecommendToBucketAction,
  deleteBucketRecommendation,
...
const PostItem = ({ item, isSinglePost }) => {
  ...
  const [upvoteCount, setUpvoteCount] = useState(
    item && item.upvotes ? item.upvotes.length : 0
  );
  let [isUpvoted, setIsUpvoted] = useState(
    item && user && item.upvotes ? item.upvotes.includes(user._id) : false
  );
  const [isHover, setHover] = useState(false);
  const history = useHistory();
  const isMyPost =
    user && user.recommendations
      ? new Set(user.recommendations).has(item._id)
      : false;
  const [haveIBucketed, setHaveIBucketed] = useState(
    user && user.bucket ? new Set(user.bucket).has(item._id) : false
  );
      ...
      <div>
        <div
          ...
          style={{
            ...
            borderRightWidth: 0,
            borderStyle: 'solid',
            borderImage: `linear-gradient(to right, ${colors[0]} 50%, ${
          ...
          <div className='list-container' id={item._id}>
            ...
            <div className='section'>
              <div className='section-icons'>
                  ...
                  <UpCircleFilled
                    ...
                    onClick={() => {
                      if (user) {
                        ...
                        upvoteDownvoteLocal();
                        dispatch(
                          removeUpvoteRecommendation(item._id, {
                            type: 'upvote',
                  ...
                  <UpCircleOutlined
                    ...
                    onClick={() => {
                      if (user) {
                        ...
                        upvoteDownvoteLocal();
                        dispatch(
                          upvoteRecommendation(item._id, notification, isMyPost)
                        );
                      } else {
                        dispatch(
                          alertMessageAction({
                  ...
                  <FolderOutlined
                    ...
                    onClick={() => {
                      if (user) {
                        ...
                        dispatch(
                          alertMessageAction({
                            message: `${item.title} added to bucket`,
                            type: 'success',
                          })
                        );
                      } else {
                ...
                {isSinglePost && isMyPost && (
                  <DeleteOutlined
                    onClick={() => {
                      dispatch(
                        warningCardRequestAction({ type: 'list', id: item._id })
                      );
                    }}
            ...
            >
              ...
              <div className='section-2'>
                ...
                >
                  {item && item.ownerUserName && (
                    <p>
                      {item.ownerUserName.split(' ')[0].length <= 9
                        ? item.ownerUserName.split(' ')[0]
                        : item.ownerUserName
                            .split(' ')[0]
                            .substr(0, 9)
                            .toString() + '..'}
